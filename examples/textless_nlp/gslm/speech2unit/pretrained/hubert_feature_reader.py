...
class HubertFeatureReader:
    ...
    def get_feats(self, input_data, ref_len=None, channel_id=None):
        if isinstance(input_data, str):
            x = self.read_audio(input_data, ref_len, channel_id)
        else:
            x = input_data
            assert len(x.shape) == 1, "Expected 1D audio array"
            assert isinstance(x, np.ndarray), "Expected numpy array for raw audio input"
            assert x.shape[0] > 0, "Expected non-empty audio array"
...
#             [checkpoint_path]
#         )
#         self.model = model[0].eval()  # Set model to evaluation mode
#         self.task = task
#         self.layer = layer
#         self.max_chunk = max_chunk
#         self.use_cuda = use_cuda
...
#             self.model.cuda()
#         else:
#             self.use_cuda = False  # Override in case CUDA is not available
