...
def extract_syllables_with_tones(phones_tier, vowel_set):
    ...
    current_plain_vowels = []  # New list to store vowels without tones
    start_time = None
    tones = []
    ...
    def strip_tone(vowel):
        if vowel.endswith('H') or vowel.endswith('L'):
            return vowel[:-1]  # Remove the last character (H or L)
        return vowel
    ...
    for interval in phones_tier.entryList:
        ...
        if phone in vowel_set:  # If the phone is a vowel, end of syllable
            ...
            current_plain_vowels.append(strip_tone(phone))  # Add vowel without tone
            ...
            syllables.append({
                ...
                'plain_vowels': current_plain_vowels  # Include vowels without tones
            })
            current_syllable = []
            current_vowels = []  # Reset vowels for the next syllable
            current_plain_vowels = []  # Reset plain vowels for the next syllable
...
def get_features_and_labels(
    ...
    plain_vowels_list = []  # List to store vowels without tones
    ...
    for file_path, segments in tqdm.tqdm(iterator, total=num_files):
        for features, syllable, tone, segment in segments:
            ...
            if len(features) > 0:  # Check if features were extracted
                ...
                plain_vowels_list.append(segment['plain_vowels'])  # Append plain vowels
            else:
                reason = "Feature array is empty"
                print(f"No features extracted for this segment in file {file_path}: {segment} - Reason: {reason}")
    ...
    return features_list, labels_list, tones_list, syllables_list, vowels_list, plain_vowels_list # Return vowels_list
...
def get_and_dump_features_and_labels(
    ...
    # Feature, label, and syllable extraction
    features, labels, tones, syllables, vowels, plain_vowels = get_features_and_labels(  # Updated function call
        feature_type=feature_type,
        checkpoint_path=checkpoint_path,
        layer=layer,
        manifest_path=manifest_path,
    ...
    np.save(out_labels_path.replace("labels", "plain_vowels"), np.array(plain_vowels, dtype=object))  # Save plain vowels
    ...
    plain_vowels_csv_path = out_labels_path.replace("labels.npy", "plain_vowels.csv")  # CSV file for plain vowels
    ...
    save_to_csv(plain_vowels, plain_vowels_csv_path, header=["Plain Vowels"])  # Save plain vowels to CSV

    return features, labels, syllables, tones, vowels, plain_vowels  # Return vowels
...
if __name__ == "__main__":
    ...
    features, labels, syllables, tones, vowels, plain_vowels = get_and_dump_features_and_labels(  # Updated function call
    ...
    logger.info(f"Saved extracted plain vowels at {args.out_labels_path.replace('labels', 'plain_vowels')}")  # New log for plain vowels
