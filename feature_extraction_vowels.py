...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=320,
        help="Downsample factor of the feature reader",
    )
    return parser
...
def get_feature_iterator(
    feature_type, checkpoint_path, layer, manifest_path, sample_pct, alignments, downsample_factor, min_length=4000
):
            ...
            for file_path in file_path_list:
                ...
                if segments:
                    ...
                    for label, start, end in segments:
                        start_frame = int(start * sample_rate / downsample_factor)
                        end_frame = int(end * sample_rate / downsample_factor)
                        segment_feats = feats[start_frame:end_frame]
                        if segment_feats.size >= min_length:
                            yield segment_feats, collapse_vowel(label)
