...
def process_file(file_path, alignments, feature_extractor, model, layer, min_input_size, pooling_type):
    ...
    if segments:
        ...
        for segment in segments:
            ...
            results.append((pooled_feats.cpu().numpy(), segment['phone'], segment['tone'], segment['plain_vowel']))
    return results
    ...
    with open(manifest_path, "r") as fp:
        ...
        def iterate():
            with ThreadPoolExecutor(max_workers=4) as executor:  # Adjust max_workers as per your system capabilities
                futures = [
                    executor.submit(
                        process_file, file_path, alignments, feature_extractor, model, layer, min_input_size, pooling_type
                    )
...
def get_features_and_labels(
    ...
    for segments in tqdm.tqdm(iterator, total=num_files):
        for features, vowel, tone, plain_vowel in segments:
            total_segments += 1
            ...
            if features.size > 0:
                ...
                print(f"No features extracted for this segment - Reason: Feature array is empty")
    ...
    return features_list, labels_list, tones_list, vowels_list, plain_vowels_list
...
def get_and_dump_features_and_labels(
    ...
    features, labels, tones, vowels, plain_vowels = get_features_and_labels(
        feature_type=feature_type,
        model_name=model_name,
        layer=layer,
        manifest_path=manifest_path,
    ...
    # save_to_csv(labels, labels_csv_path, header=["Vowel"])
    ...
    return features, labels, tones, vowels, plain_vowels
...
if __name__ == "__main__":
    ...
    features, labels, tones, vowels, plain_vowels = get_and_dump_features_and_labels(
